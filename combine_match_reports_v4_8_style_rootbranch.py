#!/usr/bin/env python3
"""
combine_match_reports_v4_8_style_rootbranch.py

Run-level combiner for match reports produced by make_wdl_report_v4_8_style_rootbranch.py, v4_5/v4_3 CCP reporters, or earlier v4 difficulty reporters.
...
Main outputs:
  ...
  - run_ccp_v2_root_branch_caused_summary.csv       (if v4.8 root-branch caused summaries exist)
  - run_style_pressure_summary.csv                  (if v4.8 style-pressure summaries exist)
...
ROOT_BRANCH_FILE_STEMS = {
    ...
    "root_branch_caused_summary": "ccp_v2_root_branch_caused_summary",
    "style_pressure_summary": "style_pressure_summary",
}
...
def write_markdown(outpath: Path, run_name: str, by_match: pd.DataFrame, overall: pd.DataFrame,
                   ...
                   root_branch_workload_rows: Optional[pd.DataFrame] = None,
                   root_branch_caused_summary: Optional[pd.DataFrame] = None,
                   style_pressure_summary: Optional[pd.DataFrame] = None) -> None:
    lines: List[str] = []
    ...
    if style_pressure_summary is not None and not style_pressure_summary.empty:
        lines.append("## Style / Pressure Summary by Match")
        lines.append("")
        lines.append("This v4.8 table gathers the most article-facing pressure/style columns from each match report.")
        lines.append("")
        style_cols = [
            "match_no", "match_name", "player", "score_sum", "expected_score_sum", "conversion_sum",
            ...
            "ccp_v2_log_search_difficulty_soft_pressure_yield_caused",
            "root_confirm_caused_mean_branch_score_log", "root_confirm_caused_ccp_rate",
            "root_confirm_caused_quiet_rate", "root_confirm_caused_positive_payoff_rate",
        ]
        lines.append(md_table(style_pressure_summary, style_cols, max_rows=20, digits=4))
        lines.append("")
    ...
    if root_branch_caused_summary is not None and not root_branch_caused_summary.empty:
        lines.append("## CCP v2 Root-Branch Caused Summary Across Run")
        lines.append("")
        lines.append("These rows group root-branch search-shape diagnostics by the player who caused the opponent's root task.")
        lines.append("")
        caused_cols = [
            "match_no", "match_name", "scope", "rows", "unique_plies", "branches_per_ply",
            "mean_branch_score_log", "positive_payoff_rate", "ccp_rate", "quiet_rate",
            "mean_child_leaf_count", "mean_child_max_chain_length",
        ]
        lines.append(md_table(root_branch_caused_summary, caused_cols, max_rows=40, digits=4))
        lines.append("")
...
def main() -> None:
    ...
    run_root_branch_caused_summary = combine_optional_match_tables(groups, "root_branch_caused_summary", args.run_name)
    run_style_pressure_summary = combine_optional_match_tables(groups, "style_pressure_summary", args.run_name)
    ...
    if not run_root_branch_caused_summary.empty:
        run_root_branch_caused_summary.to_csv(outdir / "run_ccp_v2_root_branch_caused_summary.csv", index=False)
    if not run_style_pressure_summary.empty:
        run_style_pressure_summary.to_csv(outdir / "run_style_pressure_summary.csv", index=False)
    write_markdown(
        ...
        run_root_branch_caused_summary,
        run_style_pressure_summary,
    )
    ...
    if not run_root_branch_caused_summary.empty:
        print(f"Root branch caused:       {outdir / 'run_ccp_v2_root_branch_caused_summary.csv'}")
    if not run_style_pressure_summary.empty:
        print(f"Style pressure summary:   {outdir / 'run_style_pressure_summary.csv'}")
