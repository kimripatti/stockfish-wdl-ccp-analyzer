#!/usr/bin/env python3
"""
make_wdl_report_v4_8_style_rootbranch.py
...
Main outputs:
  ...
  - ccp_v2_root_branch_caused_summary.csv     (root-branch search-shape summarized by pressure-causer)
  - style_pressure_summary.csv                (compact player style/pressure summary)
...
Usage:
python make_wdl_report_v4_8_style_rootbranch.py \
...
def first_nonempty_value(values: Iterable[Any]) -> Any:
    """Return the first non-empty, non-NaN value from an iterable."""
    for v in values:
        if v is None:
            continue
        try:
...
def supplement_san_from_root_branches(per_move_named: pd.DataFrame, root_branches: Optional[pd.DataFrame]) -> pd.DataFrame:
    """Fill played/best SAN in per-move tables from root-branch diagnostics when available.

    The reporter can already generate SAN from PGN. When no PGN is supplied, v3.6+
    root-branch diagnostics may still contain `played_move_san` and `best_move_san`.
    This function uses those as a harmless fallback so critical-move tables are more
    article-ready.
    """
    if root_branches is None or root_branches.empty or not {"game_id", "ply"}.issubset(root_branches.columns):
        return per_move_named
    rb = root_branches.copy()
    rows: List[Dict[str, Any]] = []
    for (gid, ply), g in rb.groupby(["game_id", "ply"], dropna=True):
        played_san = first_nonempty_value(g.get("played_move_san", []))
        best_san = first_nonempty_value(g.get("best_move_san", []))
        best_uci = first_nonempty_value(g.get("best_move_uci", []))
        if not played_san and "root_played_move" in g.columns:
            gg = g[g["root_played_move"].map(truthy_value)]
            played_san = first_nonempty_value(gg.get("move_san", []))
        rows.append({
            "game_id": gid,
            "ply": ply,
            "_rb_played_move_san": played_san,
            "_rb_best_move_san": best_san,
            "_rb_best_move_uci": best_uci,
        })
    if not rows:
        return per_move_named
    san_df = pd.DataFrame(rows)
    out = per_move_named.merge(san_df, on=["game_id", "ply"], how="left")
    for dst, src in [("move_san", "_rb_played_move_san"), ("best_move_san", "_rb_best_move_san"), ("best_move_uci", "_rb_best_move_uci")]:
        if dst not in out.columns:
            out[dst] = ""
        if src in out.columns:
            mask = out[dst].isna() | (out[dst].astype(str).str.strip() == "")
            out.loc[mask, dst] = out.loc[mask, src].fillna("")
    return out.drop(columns=[c for c in ["_rb_played_move_san", "_rb_best_move_san", "_rb_best_move_uci"] if c in out.columns])


def enrich_root_branch_causation(root_branches: pd.DataFrame, per_move_named: pd.DataFrame) -> pd.DataFrame:
    """Attach one-ply caused-by fields to every root-branch diagnostic row."""
    if root_branches is None or root_branches.empty or not {"game_id", "ply"}.issubset(root_branches.columns):
        return root_branches
    causation_cols = [
        "game_id", "ply", "difficulty_caused_by_player", "caused_by_mover",
        "caused_by_move_uci", "caused_by_move_san",
    ]
    cols = [c for c in causation_cols if c in per_move_named.columns]
    if len(cols) < 2:
        return root_branches
    cause = per_move_named[cols].drop_duplicates(subset=["game_id", "ply"])
    out = root_branches.copy()
    # Avoid duplicate *_x/*_y columns if the analyzer eventually writes these directly.
    for c in cols:
        if c not in {"game_id", "ply"} and c in out.columns:
            out = out.drop(columns=[c])
    return out.merge(cause, on=["game_id", "ply"], how="left")


def summarize_root_branch_group(scope: str, g: pd.DataFrame) -> Dict[str, Any]:
    """Shared summary helper for faced and caused root-branch diagnostics."""
    if g.empty:
        return {"scope": scope, "rows": 0}
    unique_plies = g[["game_id", "ply"]].drop_duplicates().shape[0] if {"game_id", "ply"}.issubset(g.columns) else None
    rows = len(g)
...
def root_branch_caused_summary(root_branches: pd.DataFrame) -> pd.DataFrame:
    """Summarize root-branch diagnostics by player who caused the opponent's root task."""
    if root_branches is None or root_branches.empty or "difficulty_caused_by_player" not in root_branches.columns:
        return pd.DataFrame()
    df = add_root_branch_derived(root_branches)
    df = df[df["difficulty_caused_by_player"].notna() & (df["difficulty_caused_by_player"].astype(str).str.strip() != "")]
    if df.empty:
        return pd.DataFrame()
    rows: List[Dict[str, Any]] = []
    for player, g in df.groupby("difficulty_caused_by_player", dropna=True):
        rows.append(summarize_root_branch_group(f"caused_by:{player}", g))
    if "ccp_v2_stage" in df.columns:
        for (player, stage), g in df.groupby(["difficulty_caused_by_player", "ccp_v2_stage"], dropna=True):
            rows.append(summarize_root_branch_group(f"caused_by:{player}|stage:{stage}", g))
    return pd.DataFrame(rows)


def _summary_scope_lookup(summary: pd.DataFrame, scope: str) -> Dict[str, Any]:
    if summary is None or summary.empty or "scope" not in summary.columns:
        return {}
...
def build_style_pressure_summary(player_summary: pd.DataFrame,
                                 root_branch_summary: Optional[pd.DataFrame] = None,
                                 root_branch_caused: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """Compact player-level style and pressure table.

    This is intentionally redundant with player_summary and root-branch tables:
    it gathers the most interpretation-heavy columns into one article-facing file.
    """
    if player_summary is None or player_summary.empty or "player" not in player_summary.columns:
        return pd.DataFrame()
    base_cols = [
        "player", "games", "moves", "score_sum", "expected_score_sum", "conversion_sum",
        "accuracy_wdl_mean", "mean_es_loss", "rms_es_loss", "volatility_wdl_mean",
        ...
        "ccp_search_difficulty_soft_caused_share", "ccp_search_difficulty_soft_dpq",
        "ccp_search_difficulty_soft_pressure_yield_suffered", "ccp_search_difficulty_soft_pressure_yield_caused",
        "ccp_quiet_to_ccp_burden_faced_mean", "ccp_quiet_to_ccp_burden_caused_mean",
        "ccp_v2_log_search_difficulty_soft_faced_mean", "ccp_v2_log_search_difficulty_soft_caused_mean",
        ...
        "ccp_v2_quiet_pause_burden_faced_mean", "ccp_v2_quiet_pause_burden_caused_mean",
        "narrowness_faced_mean", "narrowness_caused_mean",
        "effective_good_moves_faced_mean", "effective_good_moves_caused_mean",
    ]
    rows: List[Dict[str, Any]] = []
    for _, r in player_summary.iterrows():
        player = str(r.get("player", ""))
        out = {c: r.get(c) for c in base_cols if c in player_summary.columns}
        faced = _summary_scope_lookup(root_branch_summary, f"player:{player}|stage:confirm")
        caused = _summary_scope_lookup(root_branch_caused, f"caused_by:{player}|stage:confirm")
        for prefix, src in [("root_confirm_faced", faced), ("root_confirm_caused", caused)]:
            for k in [
                "branches_per_ply", "mean_branch_score_log", "mean_child_leaf_count",
                "mean_child_max_chain_length", "mean_child_quiet_pause_burden",
                "positive_payoff_rate", "ccp_rate", "quiet_rate", "played_move_row_rate",
            ]:
...
def write_markdown_report(outpath: Path, match_name: str, all_metrics: pd.DataFrame, player_summary: pd.DataFrame,
                          ...
                          ccp_v2_root_branch_caused_summary: pd.DataFrame,
                          style_pressure_summary: pd.DataFrame) -> None:
    ...
    lines.append("- Root-branch caused summaries attribute the opponent's root-branch search task one ply backward to the player who created it.")
    lines.append("- style_pressure_summary.csv gathers the most article-facing pressure/style columns into one compact table.")
    ...
    if style_pressure_summary is not None and not style_pressure_summary.empty:
        lines.append("## Style / Pressure Summary")
        lines.append("")
        lines.append("_Compact player-facing table for comparing pressure created, pressure suffered, DPQ, Pressure Yield, and root-branch search style._")
        lines.append("")
        style_cols = [
            "player", "score_sum", "expected_score_sum", "conversion_sum",
            "accuracy_wdl_mean", "mean_es_loss", "rms_es_loss",
            "difficulty_soft_uncertainty_caused_share", "difficulty_soft_uncertainty_dpq",
            "difficulty_soft_uncertainty_pressure_yield_suffered", "difficulty_soft_uncertainty_pressure_yield_caused",
            "ccp_search_difficulty_pure_caused_share", "ccp_search_difficulty_pure_dpq",
            "ccp_search_difficulty_pure_pressure_yield_suffered", "ccp_search_difficulty_pure_pressure_yield_caused",
            "ccp_v2_log_search_difficulty_soft_caused_share", "ccp_v2_log_search_difficulty_soft_dpq",
            "ccp_v2_log_search_difficulty_soft_pressure_yield_suffered", "ccp_v2_log_search_difficulty_soft_pressure_yield_caused",
            "root_confirm_caused_mean_branch_score_log", "root_confirm_caused_ccp_rate", "root_confirm_caused_quiet_rate",
            "root_confirm_caused_positive_payoff_rate", "root_confirm_caused_mean_child_leaf_count",
        ]
        lines.append(df_to_markdown(select_columns(style_pressure_summary, style_cols), digits=4))
        lines.append("")
    ...
    if ccp_v2_root_branch_caused_summary is not None and not ccp_v2_root_branch_caused_summary.empty:
        lines.append("## CCP v2 Root-Branch Caused Summary")
        lines.append("")
        lines.append("_Same root-branch search-shape diagnostics, but grouped by the player who caused the opponent's root task one ply earlier._")
        lines.append("")
        lines.append(df_to_markdown(ccp_v2_root_branch_caused_summary, digits=4))
        lines.append("")
    ...
    lines.append("- Root-branch caused summaries make the same branch-shape information usable as a style metric: faced rows describe what a player had to solve; caused rows describe what that player made the opponent solve.")
    lines.append("- style_pressure_summary.csv is the fastest file for article comparison: it combines WDL result quality, pressure caused share, DPQ, Pressure Yield, and root-branch faced/caused diagnostics.")
    lines.append("")
...
def main() -> None:
    ...
    root_branches_raw = None
    if root_branches_path:
        root_branches_raw = add_root_branch_derived(pd.read_csv(root_branches_path))
        per_move_named = supplement_san_from_root_branches(per_move_named, root_branches_raw)
    ...
    if root_branches_raw is not None:
        root_branches = enrich_root_branch_causation(root_branches_raw, per_move_named)
        ...
        ccp_v2_root_branch_caused_summary = root_branch_caused_summary(root_branches)
    else:
        ccp_v2_root_branch_summary = pd.DataFrame()
        ccp_v2_root_branch_ply_summary = pd.DataFrame()
        ccp_v2_root_branch_top_moves = pd.DataFrame()
    ...
    style_pressure_summary = build_style_pressure_summary(
        player_summary, ccp_v2_root_branch_summary, ccp_v2_root_branch_caused_summary
    )
    ...
    if not ccp_v2_root_branch_caused_summary.empty:
        ccp_v2_root_branch_caused_summary.to_csv(outdir / "ccp_v2_root_branch_caused_summary.csv", index=False)
    if not style_pressure_summary.empty:
        style_pressure_summary.to_csv(outdir / "style_pressure_summary.csv", index=False)
    ...
    write_markdown_report(
        ...
        ccp_v2_root_branch_caused_summary,
        style_pressure_summary,
    )
    ...
    if not ccp_v2_root_branch_caused_summary.empty:
        print(f"Root-branch caused summary: {outdir / 'ccp_v2_root_branch_caused_summary.csv'}")
    if not style_pressure_summary.empty:
        print(f"Style pressure summary:     {outdir / 'style_pressure_summary.csv'}")
